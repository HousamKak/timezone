...
import styles from '../trade-history/TradeHistoryTable.module.css';

interface TradeRequestsTableProps {
...
export const TradeRequestsTable = ({ 
  recommendations, 
  loading, 
  onApprove, 
  onReject 
}: TradeRequestsTableProps) => {
  const handleApprove = (recommendation: RecommendationResponse) => {
    tradeHistoryStore.updateTradeStatus(recommendation.id!, 'Approved');
    onApprove?.(recommendation.id!);
  };

  const handleReject = (recommendation: RecommendationResponse) => {
    tradeHistoryStore.updateTradeStatus(recommendation.id!, 'Rejected');
  ...
  const statusTemplate = (recommendation: RecommendationResponse) => {
    ...
    return (
      <Tag 
        ...
        severity={getSeverity(recommendation.status || 'Proposed')}
      />
    );
  };
